# Stage 1 - build openssl1.1.1, sqlite3 and python with --enable-shared
...
# Stage 2 - patch rpath for python executable and distribution libs
FROM centosdev:python3 AS patch_to_make_relocatable
COPY --from=openssl_sqlite_builder /opt/python3.9 /opt
...
# do the same thing for *.so files in lib-dynload
...
COPY --from=openssl_sqlite_builder /opt/python3.9 /opt/very/relocated
...
FROM openssl_sqlite_builder AS final_archive_env
RUN cd /opt && \
