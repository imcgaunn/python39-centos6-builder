...
ARG PYTHON_BUILD_DEFINITION
ARG PYTHON_MINOR
RUN scl enable devtoolset-7 "python-build --verbose ${PYTHON_BUILD_DEFINITION} /opt/python${PYTHON_MINOR}"
...
RUN find /opt/python${PYTHON_MINOR}/lib/python${PYTHON_MINOR}/lib-dynload -name "*.so" | xargs -n1 patchelf --set-rpath '$ORIGIN/../..'
...
RUN cd /opt && \
  ...
  echo "python build complete - tar created at /opt/python${PYTHON_BUILD_DEFINITION}.tar.gz"
